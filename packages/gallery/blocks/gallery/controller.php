...
*/
	...
	protected $btTable = 'btGallery'; //Must be the same as table name in db.xml.
	                                          //Should correspond to block's directory/class name.
	
	public function getBlockTypeName() {
		return t('Gallery'); //Appears in "Add Block" list when adding blocks to a page
	}
	
	public function getBlockTypeDescription() {
		return t('Gallery'); //Only appears in dashboard "Add Functionality" page
	}
...
**************************************************************************************************/
	...
	static function getFilesetImages($fsID, $randomize = false) {
		...
		if ($randomize) {
			...
			$fl->sortByMultiple('fsDisplayOrder desc', 'fID asc'); //Requires 5.4.1 or higher: version_compare(APP_VERSION, '5.4.1', '>=');
		}
		$files = $fl->get();
		return $files;	
	}
	...
	private function processImageFiles($imageFiles, $largeWidth, $largeHeight, $cropLarge, $thumbWidth, $thumbHeight, $cropThumb) {
		...
		foreach ($imageFiles as $f) {
			...
			$image->category = $f->getAttribute('category');
